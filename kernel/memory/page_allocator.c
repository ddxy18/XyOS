...
pointer_t k_request_page(uint32_t);
 ...
 *
 * @return pointer_t Return physical address of the allocated page. if a
 * failure(request allocated page or reserved page) happens, return -1.
 */
pointer_t k_request_page(uint32_t n) {
...
void release_page(uint32_t n) {
  pointer_t addr = page_phys_addr(n);
  avl_phys_mem_t *t = get_avl_mem();
  while (t->size != -1) {
...
void clear_page(uint32_t n) {
  pointer_t physical_addr = page_phys_addr(n);
  pointer_t *byte_pointer = (pointer_t *)physical_addr;
  for (; (pointer_t)byte_pointer < physical_addr + PAGE_SIZE; ++byte_pointer) {
    *byte_pointer = 0;
  }
}